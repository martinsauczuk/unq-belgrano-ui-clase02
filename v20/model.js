...
/**
* El observador que maneja el evento que ocurre cuando la lista de mensajes se modifica.
*/
 ...
 *
 * Vamos a agregar unos mensajes desde el inicio, de forma tal que puedan
 * visualizarse ni bien inicia la aplicación. En la realidad debería comenzar
 * vacío, pero aún no sabemos si funciona el envío y recepción de mensajes de
 * forma adecuada, por lo que es mejor empezar con algo de contenido.
 */
const messages = [
...
// Agregamos el atributo observe, como la función addObserver del observador
// y la llamaremos "onChange" dentro de ese objeto.
messages.onChange = (f) => messagesChangedObserver.addObserver(f);
messages.clearListeners = (f) => messagesChangedObserver.clear();
...
/* También necesitamos exportar todo
   lo que corresponde a la lógica de la aplicación,
   para poder utilizarla en la UI. Quedará más claro
   por qué lo llamamos Model más adelante. */
window.Model = {
    messages: messages,
